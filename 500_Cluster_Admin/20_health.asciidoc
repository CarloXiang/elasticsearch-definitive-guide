...
不管集群扩展到多大规模，你都会想要一个快速获取集群状态的途径。`Cluster Health` 接口充当的就是这个角色。你可以把它想象成是在一万英尺的高度鸟瞰集群。它可以告诉你安心吧一切都好，或者警告你集群某个地方有问题。
...
和 Elasticsearch 里其他 API 一样，`cluster-health` 会返回一个 JSON 响应。这让它非常便于解析，以此做自动化和告警。响应中包括和你集群有关的一些关键信息：
